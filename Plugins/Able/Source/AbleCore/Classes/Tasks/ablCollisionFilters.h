...
#endif
protected:
	UPROPERTY(EditInstanceOnly, Category = "Filter", meta = (DisplayName = "Ignore Attitude", Bitmask, BitmaskEnum = "/Script/AIModule.ETeamAttitude"))
...
class UAblCollisionFilterUniqueActors : public UAblCollisionFilter
{
	GENERATED_BODY()
public:
	UAblCollisionFilterUniqueActors(const FObjectInitializer& ObjectInitializer);
	virtual ~UAblCollisionFilterUniqueActors();
...
UCLASS(EditInlineNew, meta = (DisplayName = "Half Space", ShortToolTip = "Reduce Collision results to only the actors in front of the source (on the XY plane)."))
class UAblCollisionFilterHalfSpace : public UAblCollisionFilter
{
	GENERATED_BODY()
public:
	UAblCollisionFilterHalfSpace(const FObjectInitializer& ObjectInitializer);
	virtual ~UAblCollisionFilterHalfSpace();
