...
void FAnimNode_AbilityAnimPlayer::UpdateAssetPlayer(const FAnimationUpdateContext& Context)
{
	...
	if (m_AnimationQueue.Num())
	{
		...
		FAnimTickRecord TickRecord(const_cast<UAnimSequence*>(CurrentEntry.AnimationSequence), false, CurrentEntry.PlayRate, false, BlendValue, CurrentEntry.TimeAccumulator, CurrentEntry.MarkerTickRecord);
		...
		switch (m_BlendType)
		{
			...
			{
				FAnimTickRecord NextTickRecord(const_cast<UAnimSequence*>(NextEntry->AnimationSequence), false, NextEntry->PlayRate, false, 1.0f - BlendValue, NextEntry->TimeAccumulator, NextEntry->MarkerTickRecord);
...
void FAnimNode_AbilityAnimPlayer::Evaluate_AnyThread(FPoseContext& Output)
{
	...
	if (!m_AnimationQueue.Num())
	{
		...
		{
			m_CachedOutputSequence->GetAnimationPose(PoseData, FAnimExtractContext((double)m_CachedOutputTime, Output.AnimInstanceProxy->ShouldExtractRootMotion()));
		}
		else
		{
	...
	switch (m_BlendType)
	{
		...
		{
			FAbilityAnimEntry& CurrentEntry = m_AnimationQueue[0];
			CurrentEntry.AnimationSequence->GetAnimationPose(PoseData, FAnimExtractContext((double)CurrentEntry.TimeAccumulator, Output.AnimInstanceProxy->ShouldExtractRootMotion()));
		}
		break;
		case EvaluateBlendType::Multi:
		case EvaluateBlendType::SingleBlendOut:
...
{
	return FMath::Max<float>(AnimationSequence->GetPlayLength() - TimeAccumulator, 0.0f);
}
