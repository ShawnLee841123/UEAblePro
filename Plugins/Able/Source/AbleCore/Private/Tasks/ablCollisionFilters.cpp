...
UAblCollisionFilterUniqueActors::UAblCollisionFilterUniqueActors(const FObjectInitializer& ObjectInitializer)
: Super(ObjectInitializer)
{

}

UAblCollisionFilterUniqueActors::~UAblCollisionFilterUniqueActors()
{

}

void UAblCollisionFilterUniqueActors::Filter(const TWeakObjectPtr<const UAblAbilityContext>& Context, TArray<FAblQueryResult>& InOutArray) const
{
	for (int32 i = 0; i < InOutArray.Num(); ++i)
	{
		const FAblQueryResult& currentEntry = InOutArray[i];
...
#if WITH_EDITOR
EDataValidationResult UAblCollisionFilterUniqueActors::IsAbilityDataValid(const UAblAbility* AbilityContext, TArray<FText>& ValidationErrors)
{
	return EDataValidationResult::Valid;
}
#endif

UAblCollisionFilterHalfSpace::UAblCollisionFilterHalfSpace(const FObjectInitializer& ObjectInitializer)
: Super(ObjectInitializer)
{

}

UAblCollisionFilterHalfSpace::~UAblCollisionFilterHalfSpace()
{

}

void UAblCollisionFilterHalfSpace::Filter(const TWeakObjectPtr<const UAblAbilityContext>& Context, TArray<FAblQueryResult>& InOutArray) const
{
	const AActor* SelfActor = Context->GetSelfActor();

	const FVector ActorFwdXY = SelfActor->GetActorForwardVector().GetSafeNormal2D();
	const FVector& SelfLoc = SelfActor->GetActorLocation();
	FVector ToTargetXY;
	for (int32 i = 0; i < InOutArray.Num();)
	{
		const FAblQueryResult& currentEntry = InOutArray[i];

		ToTargetXY = (currentEntry.GetLocation() - SelfLoc).GetSafeNormal2D();

		if (FVector::DotProduct(ActorFwdXY, ToTargetXY) < 0.0f) // Target is behind us.
		{
			// Swap this entry with the last entry, decrement the count - saves shifting the array around.
			InOutArray[i] = InOutArray.Last();
			InOutArray.RemoveAt(InOutArray.Num() - 1, 1, false);
		}
		else
		{
...
#if WITH_EDITOR
EDataValidationResult UAblCollisionFilterHalfSpace::IsAbilityDataValid(const UAblAbility* AbilityContext, TArray<FText>& ValidationErrors)
{
	return EDataValidationResult::Valid;
}
#endif