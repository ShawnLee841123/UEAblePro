...
#endif
			// Run negative query shape, remove overlapping results.
			if (m_NegativeQueryShape)
			{
				TArray<FAblQueryResult> negativeResults;
				m_NegativeQueryShape->DoQuery(Context, negativeResults);

				for (FAblQueryResult& result : negativeResults)
				{
					Results.RemoveAll([&](const FAblQueryResult& LHS)
						{
							return LHS.Actor == result.Actor || LHS.PrimitiveComponent == result.PrimitiveComponent;
						});
				}
			}
...
void UAblCollisionQueryTask::OnAbilityEditorTick(const UAblAbilityContext& Context, float DeltaTime) const
{
	...
	if (m_NegativeQueryShape)
	{
		m_NegativeQueryShape->OnAbilityEditorTick(Context, DeltaTime);
	}
}
